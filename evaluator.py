...
class PRAEvaluator:
    ...
    def __init__(self, config, tokenizer):
        ...
        self.metric2func = {
            ...
            'length': self.length_at_k,
            'IoI_per_item': self.IoI_per_item_at_k,
            'IoR_per_item': self.IoR_per_item_at_k,
    ...
    def length_at_k(self, acc_index, hit_index, len_index, len_gt_index, error_index, coherence_index, IoI_index,
                    IoR_index, ctr_index, k):
        return torch.max(len_index[:, :k], dim=1)[0]

    def IoI_per_item_at_k(self, acc_index, hit_index, len_index, len_gt_index, error_index, coherence_index,
                          IoI_index, IoR_index, ctr_index, k):
        values = IoI_index[:, :k] / len_index[:, :k].clamp(min=1.0)
        return torch.max(values, dim=1)[0]

    def IoR_per_item_at_k(self, acc_index, hit_index, len_index, len_gt_index, error_index, coherence_index,
                          IoI_index, IoR_index, ctr_index, k):
        values = IoR_index[:, :k] / len_index[:, :k].clamp(min=1.0)
        return torch.max(values, dim=1)[0]
