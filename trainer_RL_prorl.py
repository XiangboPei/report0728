...
from prorl_variants import (
    METHOD_MODES,
    EliteTrajectoryMemory,
    bounded_normalization,
    curriculum_sample_weights,
    masked_pearson_correlation,
    normalized_target_quality,
    pareto_front_scores,
    pareto_rollout_scores,
)
from utils import *
...
class PRATrainer_ProRL:
    ...
    def __init__(self, config: dict, model, tokenizer, train_dataloader: DataLoader):
        ...
        # Report methods are mutually exclusive controlled components.
        self.method_mode = config.get('method_mode', 'none')
        if self.method_mode not in METHOD_MODES:
            raise ValueError(
                f"Unsupported method_mode={self.method_mode!r}; "
                f"expected one of {list(METHOD_MODES)}"
            )
        self.pareto_weight = float(config.get('pareto_weight', 1.0))
        self.pareto_objective_mode = config.get('pareto_objective_mode', 'sum')
        if self.pareto_objective_mode not in {'sum', 'mean'}:
            raise ValueError('pareto_objective_mode must be sum or mean')
        self.pareto_control = config.get('pareto_control', 'normal')
        if self.pareto_control not in {'normal', 'shuffled', 'reverse', 'front_replace'}:
            raise ValueError(
                'pareto_control must be normal, shuffled, reverse, or front_replace'
            )
        self.memory_weight = float(config.get('memory_weight', 0.1))
        self.curriculum_start = float(config.get('curriculum_start', 0.5))
        self.curriculum_temperature = float(config.get('curriculum_temperature', 0.25))
        self.robust_scale = float(config.get('robust_scale', 2.5))

        if min(self.pareto_weight, self.memory_weight) < 0:
            raise ValueError('variant loss and advantage weights must be non-negative')

        self.elite_memory = None
        if self.method_mode == 'elite_memory':
            self.elite_memory = EliteTrajectoryMemory(config.get('memory_capacity', 4096))
    ...
    def compute_fused_rewards(self, item_ioi, item_ior, item_ctr,
                              num_items, duplicate_masks, valid_item_mask):
        """
        Args:
        ...
        if self.method_mode == 'robust_normalization':
            normalized_dict = bounded_normalization(normalized_dict, self.robust_scale)
    ...
    def compute_item_rewards(self, generated_sequences, batch):
        ...
        item_ioi, item_ior, item_ctr, reward_info = self.reward_computer.compute_rewards_batch(
        ...
        metrics = {
            ...
            'items_tensor': items_tensor,
            'initial_rank': reward_info['initial_rank'],
            'catalog_size': reward_info['catalog_size'],
        }
    ...
    def compute_policy_loss(self, batch):
        ...
        position_advantages, position_active_mask, first_eos_mask = self.compute_position_advantages(
            fused_rewards,
            structures,
            metrics['valid_mask']
        )

        if self.method_mode == 'pareto_advantage':
            pareto_ioi = metrics['sequence_ioi']
            pareto_ior = metrics['sequence_ior']
            if self.pareto_objective_mode == 'mean':
                item_counts = metrics['valid_item_counts'].clamp(min=1.0)
                pareto_ioi = pareto_ioi / item_counts
                pareto_ior = pareto_ior / item_counts
            if self.pareto_control == 'front_replace':
                pareto_scores = pareto_front_scores(
                    pareto_ioi,
                    pareto_ior,
                    metrics['sequence_ctr'],
                    metrics['valid_mask'],
                )
            else:
                pareto_scores = pareto_rollout_scores(
                    pareto_ioi,
                    pareto_ior,
                    metrics['sequence_ctr'],
                    metrics['valid_mask'],
                )
            if self.pareto_control == 'shuffled':
                pareto_scores = torch.roll(pareto_scores, shifts=1, dims=1)
                pareto_scores = pareto_scores * metrics['valid_mask'].float()
                valid_count = metrics['valid_mask'].float().sum(dim=1, keepdim=True).clamp(min=1.0)
                pareto_scores = pareto_scores - (
                    pareto_scores.sum(dim=1, keepdim=True) / valid_count
                )
                pareto_scores = pareto_scores * metrics['valid_mask'].float()
            elif self.pareto_control == 'reverse':
                pareto_scores = -pareto_scores
            variant_stats['pareto_length_corr'] = masked_pearson_correlation(
                pareto_scores, metrics['valid_item_counts'], metrics['valid_mask']
            ).item()
            variant_stats['pareto_ioi_corr'] = masked_pearson_correlation(
                pareto_scores, metrics['sequence_ioi'], metrics['valid_mask']
            ).item()
            variant_stats['pareto_ior_corr'] = masked_pearson_correlation(
                pareto_scores, metrics['sequence_ior'], metrics['valid_mask']
            ).item()
            variant_stats['pareto_ctr_corr'] = masked_pearson_correlation(
                pareto_scores, metrics['sequence_ctr'], metrics['valid_mask']
            ).item()
            pareto_advantages = (
                self.pareto_weight * pareto_scores.unsqueeze(-1) * position_active_mask.float()
            )
            if self.pareto_control == 'front_replace':
                position_advantages = pareto_advantages
            else:
                position_advantages = position_advantages + pareto_advantages
            variant_stats['pareto_score_abs'] = pareto_scores.abs().mean().item()

        memory_sequences = None
        memory_batch_indices = None
        if self.method_mode == 'elite_memory':
            memory_sequences, memory_batch_indices = self.elite_memory.fetch(
                batch['input_ids'], seq_len, device
            )
        ...
        if self.method_mode == 'elite_memory' and memory_sequences is not None:
            replay_count = memory_sequences.shape[0]
            replay_decoder_start = torch.zeros(replay_count, 1, dtype=torch.long, device=device)
            replay_decoder_input = torch.cat(
                [replay_decoder_start, memory_sequences[:, :-1]], dim=1
            )
            replay_batch = {
                'input_ids': batch['input_ids'].index_select(0, memory_batch_indices).contiguous(),
                'attention_mask': batch['attention_mask'].index_select(0, memory_batch_indices).contiguous(),
                'decoder_input_ids': replay_decoder_input.contiguous(),
            }
            self.model.eval()
            replay_logits = self.model(replay_batch).logits
            self.model.train()
            replay_log_probs = F.log_softmax(replay_logits.float(), dim=-1)
            replay_selected = replay_log_probs.gather(
                dim=2, index=memory_sequences.unsqueeze(-1)
            ).squeeze(-1)
            replay_mask = memory_sequences != 0
            memory_loss = -(
                replay_selected * replay_mask.float()
            ).sum() / replay_mask.float().sum().clamp(min=1.0)
            auxiliary_loss = auxiliary_loss + self.memory_weight * memory_loss
            variant_stats['memory_loss'] = memory_loss.item()
            variant_stats['memory_replay_count'] = float(replay_count)
        ...
        if self.method_mode == 'difficulty_curriculum':
            sample_weights, difficulty_quantiles, competence = curriculum_sample_weights(
                metrics['initial_rank'],
                valid_mask,
                epoch=self.current_epoch,
                total_epochs=self.config.get('prorl_epochs', 50),
                start_competence=self.curriculum_start,
                temperature=self.curriculum_temperature,
            )
            variant_stats['curriculum_competence'] = competence
            variant_stats['curriculum_weight_min'] = sample_weights.min().item()
            variant_stats['curriculum_weight_max'] = sample_weights.max().item()

        # Item loss
        item_weight = item_valid_mask.float()
        if self.method_mode == 'difficulty_curriculum':
            item_weight = item_weight * sample_weights.view(-1, 1, 1)
        item_policy_loss = -(item_log_probs * item_advantages * item_weight).sum()
        item_kl_loss = (self.beta_kl * (item_log_probs - ref_item_log_probs) * item_weight).sum()

        # EOS loss
        eos_valid = valid_mask & has_eos & (eos_pos < seq_len)
        eos_weight = eos_valid.float()
        if self.method_mode == 'difficulty_curriculum':
            eos_weight = eos_weight * sample_weights.view(-1, 1)
        eos_policy_loss = -(first_eos_log_probs * eos_advantages * eos_weight).sum()
        eos_kl_loss = (self.beta_kl * (first_eos_log_probs - ref_first_eos_log_probs) * eos_weight).sum()

        if self.method_mode == 'difficulty_curriculum':
            total_count = (
                valid_mask.float() * sample_weights.view(-1, 1)
            ).sum().clamp(min=1.0)
        else:
        ...
        if self.method_mode == 'elite_memory':
            memory_quality = normalized_target_quality(
                metrics['normalized_dict'], metrics['items_mask']
            )
            memory_updates = self.elite_memory.update(
                batch['input_ids'], generated_seqs, memory_quality, valid_mask
            )
            variant_stats['memory_updates'] = float(memory_updates)
            variant_stats['memory_size'] = float(len(self.elite_memory))

        total_loss = policy_loss + kl_loss + auxiliary_loss
        if self.method_mode != 'none':
            variant_stats['auxiliary_loss'] = auxiliary_loss.item()
    ...
    def fit(self, train_dataloader, val_dataloader, epochs, epoch_bias=0):
        ...
        skip_evaluation = bool(self.config.get('skip_evaluation', False))
        ...
        if self.accelerator.is_main_process:
            ...
            self.log(f"    method_mode:           {self.method_mode}")
            if self.method_mode == 'pareto_advantage':
                self.log(f"    pareto_weight:         {self.pareto_weight}")
                self.log(f"    pareto_objective_mode: {self.pareto_objective_mode}")
                self.log(f"    pareto_control:        {self.pareto_control}")
            elif self.method_mode == 'elite_memory':
                self.log(f"    memory_weight:         {self.memory_weight}")
                self.log(f"    memory_capacity:       {self.elite_memory.capacity}")
            elif self.method_mode == 'difficulty_curriculum':
                self.log(f"    curriculum_start:      {self.curriculum_start}")
                self.log(f"    curriculum_temperature:{self.curriculum_temperature}")
            elif self.method_mode == 'robust_normalization':
                self.log(f"    robust_scale:          {self.robust_scale}")
        ...
        for epoch in range(epoch_bias, epochs + epoch_bias):
            ...
            self.current_epoch = epoch - epoch_bias
            ...
            if not skip_evaluation and (
                    epoch % 10 == 0 or epoch == (epochs + epoch_bias - 1)):

                fit_eval_split = self.config.get('fit_eval_split', 'test')
                test_results, all_results = self.evaluate_all_tokenizer(
                    val_dataloader, split=fit_eval_split, store=False
                )
                ...
                if epoch == (epochs + epoch_bias - 1) and self.accelerator.is_main_process:
                    metrics_filename = (
                        'final_metrics.json' if fit_eval_split == 'test'
                        else f'final_{fit_eval_split}_metrics.json'
                    )
                    final_metrics_path = os.path.join(self.results_dir, metrics_filename)
                    final_metrics = {
                        'dataset': self.config['dataset'],
                        'epoch': epoch + 1,
                        'split': fit_eval_split,
                        'rand_seed': self.config['rand_seed'],
                        'method_mode': self.method_mode,
                        'method_config': {
                            'pareto_weight': self.pareto_weight,
                            'pareto_objective_mode': self.pareto_objective_mode,
                            'pareto_control': self.pareto_control,
                            'memory_weight': self.memory_weight,
                            'curriculum_start': self.curriculum_start,
                            'curriculum_temperature': self.curriculum_temperature,
                    ...
                    self.log(f'Final metrics saved to {final_metrics_path}')
            ...
            if self.accelerator.is_main_process:
                ...
                for key in sorted(k for k in epoch_stats if k.startswith('pareto_')):
                    self.log(f"  {key}: {np.mean(epoch_stats[key]):.6f}")

                if epoch == (epochs + epoch_bias - 1):
                    diagnostics = {
                    ...
                    diagnostics_path = os.path.join(
                        self.results_dir, 'final_training_diagnostics.json'
                    )
                    with open(diagnostics_path, 'w', encoding='utf-8') as file:
                        json.dump(diagnostics, file, indent=2, ensure_ascii=True)
        ...
        if self.accelerator.is_main_process:
            if self.best_val_score == -1 and self.config.get('save_final_checkpoint', True):
...
class OptimizedRewardComputer_NoCOT:
    ...
    def compute_rewards_batch(self, items_tensor, items_mask, valid_mask,
        ...
        for b in range(batch_size):
            ...
            for s in range(num_samples):
                ...
                for k in range(n):
                    ...
                    is_initial_state = len(valid_prefix) == 0
                    ...
                    all_is_initial_state.append(is_initial_state)
        ...
        if total_items == 0:
            ...
            reward_info = {
                'initial_rank': torch.zeros(batch_size, num_samples, device=device),
                'catalog_size': int(self.sasrec_model.item_embedding.weight.shape[0]),
            }
            return zeros.clone(), zeros.clone(), zeros.clone(), reward_info
        ...
        if len(metadata) > 0:
            ...
            initial_mask = torch.tensor(all_is_initial_state, device=device, dtype=torch.bool)
            if initial_mask.any():
                initial_rank[
                    b_indices[initial_mask],
                    s_indices[initial_mask],
                ] = rank_before[initial_mask].float()

        reward_info = {
            'initial_rank': initial_rank,
            'catalog_size': int(prob_before.shape[1]),
        }
        return item_ioi, item_ior, item_ctr, reward_info
